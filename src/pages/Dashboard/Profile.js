...
const Profile = ({ item }) => {
    const { name, email, education, img, linkedin, location, phone } = item;
    return (
        <div class="max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex flex-col items-center pb-10">
                <img class="mb-3 w-24 h-24 rounded-full shadow-lg" src={img} alt="Bonnie image" />
                <h5 class="mb-1 text-xl font-medium text-gray-900 dark:text-white">{name}</h5>
                <span class="text-sm text-gray-500 dark:text-gray-400">Education: {education}</span>
                <span class="text-sm text-gray-500 dark:text-gray-400">Location: {location}</span>
                <span class="text-sm text-gray-500 dark:text-gray-400">Phone: {phone}</span>
                <span class="text-sm text-gray-500 dark:text-gray-400">LinkedIn: {linkedin}</span>
